  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-16">
      ...
      <header className="text-center mb-12">
        <Link to="/shop" className="inline-flex items-center text-gray-400 hover:text-white transition mb-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-gray-800 rounded-xl shadow-md p-6">
          <h2 className="text-2xl font-semibold mb-6 text-white">Datos de contacto</h2>
        ...
        {/* Order Notes */}
        <div className="bg-gray-800 rounded-xl shadow-md p-6">
          <h3 className="text-lg font-semibold mb-4 text-white">Notas para el pedido (opcional)</h3>
          <textarea 
            rows="4" 
            maxLength="200" 
        ...
        {/* Cart Items */}
        <div className="bg-gray-800 rounded-xl shadow-md p-6">
          <h2 className="text-2xl font-semibold mb-6 text-white">Resumen del pedido</h2>
          <div className="space-y-4">
            {cart.length === 0 ? (
              <p className="text-gray-400">No hay productos en el carrito.</p>
            ) : (
              cart.map((item, index) => (
                <div key={index} className="flex items-center justify-between gap-4 py-4 border-b border-gray-700/50">
                  <img src={item.imagen} alt={item.nombre} className="w-16 h-16 rounded-md object-cover" />
                  <div className="flex-1">
                    <h4 className="font-medium text-base text-white">{item.nombre}</h4>
          ...
          <div className="mt-6">
            <div className="flex justify-between py-2">
              ...
              <span className="font-medium text-white">{CLP.format(getSubtotal())}</span>
            </div>
          ...
          <label className="flex items-start gap-3 text-sm text-gray-300 mt-4">
            ...
            />
            <span>
              Acepto los <a href="/terminos" className="underline hover:no-underline">términos y condiciones</a> y confirmo que mi nombre de Fortnite es correcto.
            </span>
          </label>
