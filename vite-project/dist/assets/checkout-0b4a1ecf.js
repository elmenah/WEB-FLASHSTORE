import"./modulepreload-polyfill-3cfb730f.js";document.addEventListener("DOMContentLoaded",()=>{const o=document.querySelector("#proceed-to-payment");o&&o.addEventListener("click",a=>{a.preventDefault();const n=document.querySelector("#email").value.trim(),t=document.querySelector("#discord").value.trim(),r=document.querySelector("#fortniteusername").value.trim();if(!n||!t||!r){alert("Please fill in all the required fields.");return}let e=JSON.parse(localStorage.getItem("carrito"))||[];if(e.length===0){alert("Your cart is empty.");return}let c=e.reduce((l,i)=>l+i.precio*(i.cantidad||1),0);c=c.toLocaleString("es-CL",{style:"currency",currency:"CLP"}),d(n,t,r,c,e),alert("Processing your payment..."),setTimeout(()=>{alert("Your payment was successful! Thank you for your purchase.")},2e3)})});document.addEventListener("DOMContentLoaded",()=>{let o=JSON.parse(localStorage.getItem("carrito"))||[];function a(){const n=document.getElementById("cart-items-container");if(n.innerHTML="",o.length===0){n.innerHTML="<p>No items in cart.</p>";return}o.forEach(t=>{const r=document.createElement("div");r.classList.add("flex","items-center","justify-between","mb-4"),r.innerHTML=`
...
`});let i=`https://wa.me/56930917730?text=${encodeURIComponent(e)}`;window.open(i,"_blank")}
